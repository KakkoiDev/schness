...
test('a checked king has an unmistakable turn warning and board marker', async () => {
  ...
  assert.match(css, /#board \.square\.in-check::before \{[^}]*content: "CHECK"/);
  assert.match(css, /#board \.square\.in-check \{[^}]*background-color: var\(--check-square\);[^}]*background-image: none/);
